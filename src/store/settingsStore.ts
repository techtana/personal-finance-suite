...
interface SettingsState {
  ...
  updateSettings: (patch: Partial<AppSettings>, client?: FirestoreClient) => Promise<void>
  loadFromFirestore: (client: FirestoreClient) => Promise<void>
}
  ...
  persist(
    (set, get) => ({
      ...
      updateSettings: async (patch, client) => {
        ...
        set({ settings: next })
        if (client) await client.write('settings', { settings: next })
      },

      loadFromFirestore: async (client) => {
        try {
    ...
    { name: 'pfs:settings', partialize: (s) => ({ settings: s.settings }) },
  ),
)
